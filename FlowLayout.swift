...
struct FlowResult {
    ...
    init(in maxWidth: CGFloat, subviews: LayoutSubviews, alignment: Alignment, spacing: CGFloat) {
        let fallbackWidth = UIScreen.main.bounds.width - 32  // безопасная ширина
        let width = maxWidth.isFinite ? maxWidth : fallbackWidth
        ...
        for subview in subviews {
            ...
            if currentPosition.x + subviewSize.width > width && currentPosition.x > 0 {
                // Move to next line
                currentPosition.x = 0
